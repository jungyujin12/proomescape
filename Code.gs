...
const EVENT_WEEK_START = '2026-09-14';
...
function updateStatus_(id, status) {
  ...
  lock.waitLock(10000);
  try {
    const sheet = sheet_(), rows = values_(sheet), index = rows.findIndex(row => row[0] === String(id));
    if (index < 0) return { ok: false, message: '예약을 찾을 수 없습니다.' };
    if (status !== '취소' && rows.some((row, rowIndex) => rowIndex !== index && row[10] !== '취소' && (normalizePhone_(row[4]) === normalizePhone_(rows[index][4]) || String(row[5] || '').trim() === String(rows[index][5] || '').trim()))) return { ok: false, code: 'DUPLICATE_APPLICANT', message: '같은 전화번호 또는 학번의 활성 예약이 있어 상태를 변경할 수 없습니다.' };
    if (status !== '취소' && rows.some((row, rowIndex) => rowIndex !== index && row[1] === rows[index][1] && row[2] === rows[index][2] && row[10] !== '취소')) return { ok: false, code: 'DUPLICATE_SLOT', message: '해당 시간에는 다른 예약이 있어 상태를 변경할 수 없습니다.' };
...
function updateReservation_(id, data) {
  ...
  try {
    ...
    if (index < 0) return { ok: false, message: '예약을 찾을 수 없습니다.' };
    const phoneKey = normalizePhone_(data.phone), studentKey = String(data.studentId || '').trim();
    const duplicateApplicant = data.status !== '취소' && rows.some((row, rowIndex) => rowIndex !== index && row[10] !== '취소' && (normalizePhone_(row[4]) === phoneKey || String(row[5] || '').trim() === studentKey));
    if (duplicateApplicant) return { ok: false, code: 'DUPLICATE_APPLICANT', message: '같은 전화번호 또는 학번의 활성 예약이 이미 있습니다.' };
...
function validateReservation_(data) {
  ...
  if (!isValidEventSlot_(data.date, data.time)) throw new Error('예약 가능한 행사 일시가 아닙니다.');
...
function isValidEventSlot_(date, time) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date || '') || !/^\d{2}:\d{2}$/.test(time || '')) return false;
  const start = new Date(EVENT_WEEK_START + 'T00:00:00+09:00'), target = new Date(String(date) + 'T00:00:00+09:00');
  const dayOffset = Math.round((target.getTime() - start.getTime()) / 86400000);
  if (dayOffset < 0 || dayOffset > 4 || target.getDay() < 1 || target.getDay() > 5) return false;
  const hours = OPERATING_HOURS[target.getDay()];
  const toMinutes = value => Number(value.slice(0, 2)) * 60 + Number(value.slice(3));
  const slot = toMinutes(time), first = toMinutes(hours[0]), end = toMinutes(hours[1]);
  return slot >= first && slot < end && (slot - first) % 15 === 0;
}
